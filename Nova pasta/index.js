...
const map1 = [
    ['lu', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', 'ru'],  // descrição das bordas 'l' = left 'r' = right 'u' = up 'd' = down ('lu' = left up)
    ['|', '.', '.', '.', '.', 'p', '.', '.', '.', '.', ' ', ' ', '|'],  // 'b' = bloco 
    ['|', '.', 'b', '.', '[', '7', ']', '.', 'b', '.', '[', '-', '8'],  // 'p' = powerUP
    ['|', '.', '.', '.', '.', '_', '.', '.', '.', '.', ' ', ' ', '|'],  // Gp = ghost purple   // Gc = ghost cyan   // Gr = ghost red // Gg = ghost cyan
    ['|', 'p', '[', ']', '.', 'p', 'Pac', '[', ']', '.', '[', '-', '8'],    // Pac = Pacman
    ['|', '.', '.', '.', '.', '^', '.', '.', '.', '.', ' ', '', '|'],
    ['|', '.', 'b', '.', '[', '5', ']', '.', 'b', '.', '[', '-','8'],
    ['|', '.', '.', '.', '.', 'p', '.', '.', '.', '.', ' ', 'Gr', '|'],   
    ['ld', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', 'rd']
...
const map3 = [
    ...
    ['|', '.', '.', '.', '.', 'p', '.', '.', '.', '.', ' ', 'Gc', '|'],   // 'b' = bloco 
    ['|', '.', 'b', '.', '[', '7', ']', '.', 'b', '.', '[', '-', '8'],    // 'p' = powerUP
    ['|', '.', '.', '.', '.', '_', '.', '.', '.', '.', ' ', 'Gg', '|'],    // Gp = ghost purple   // Gc = ghost cyan   // Gr = ghost red // Gg = ghost cyan
    ['|', 'Pac', '[', ']', '.', 'p', '.', '[', ']', 'p', '[', '-', '8'],
    ['|', '.', '.', '.', '.', '^', '.', '.', '.', '.', ' ', 'Gr', '|'],
    ['|', '.', 'b', '.', '[', '5', ']', '.', 'b', '.', '[', '-','8'],
    ['|', '.', '.', '.', '.', 'p', '.', '.', '.', '.', ' ', 'Gp', '|'],   
    ...
    for(let i= 0; i < ghost.pathways.length; i ++){
        switch(ghost.pathways[i]){
            case 'down':
                nextPosisition.position = {x: ghost.position.x, y: ghost.position.y + ghost.speed}
                ghost.scared ? heuristic = -calculateHeuristic({player, ghost: nextPosisition}) : heuristic =  calculateHeuristic({player, ghost: nextPosisition}) ;
                if(heuristic <= bestheuristic+ cost){
                    bestheuristic = heuristic
                    caminho = 'down'
                }
                break
...
function mapChange({maps, currentmap}){
    mapDestructor()
...
function animate() {
    ...
    if(pellets.length === 0){
        ...
        setTimeout(() => {
            currentmap++
            currentmap < maps.length ? mapChange({maps: maps, currentmap: currentmap}) : console.log('Ganhou!')
        }, 2000)
    }
}
